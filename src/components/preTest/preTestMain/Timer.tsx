...
function Timer() {
    ...
    return (
        <T.TimerContainer>
            {status === 'loading' && <T.StatusText>로딩 중...</T.StatusText>}
            {status === 'end' && <T.StatusText>면접이 종료되었습니다.</T.StatusText>}
            {status === 'started' && <T.StatusText>면접이 시작되었습니다:)<br/>면접 응시를 시작해 주세요!</T.StatusText>}
            {status === '' && (
                <>
                    <T.TimerWrap>
